...
export default class ListItemService extends ContextService {
    ...
    public async getQuestionTextItemUsingCAML(fieldName: string,fieldName1: string, selectFields: string[], orderByXML: string, resultType: Enums.ItemResultType): Promise<any> {
       //debugger;
        //const camlFilterConditions = "<Where><Eq><FieldRef Name='Service_x0020_Line'/><Value Type='Choice'>" + fieldName + "</Value></Eq></Where>";
        const camlFilterConditions = "<Where><And><Eq><FieldRef Name='Service_x0020_Line'/><Value Type='Choice'>" + fieldName + "</Value></Eq><Eq><FieldRef Name='Role'/><Value Type='Choice'>" + fieldName1 + "</Value></Eq></And></Where>";
        const allResultItems: any = await this.getItemsUsingCAML(selectFields, orderByXML, camlFilterConditions, 1, resultType);
        if (allResultItems) {
    ...
    public async CheckEmployeeAlreadyExists(employee: User) : Promise<boolean>{
        ...
        const allResultItems: any = await this.getItemsUsingCAML(["ID"], undefined, camlFilterConditions, 1, Enums.ItemResultType.PEPI_ItemID);
