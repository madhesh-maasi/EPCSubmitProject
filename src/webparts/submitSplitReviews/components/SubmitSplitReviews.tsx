...
import { Dropdown, IDropdownOption, IStackTokens, Label, PrimaryButton, TextField, Stack, MessageBar, MessageBarType, Spinner, SpinnerSize } from '@fluentui/react';
...
export default class SubmitSplitReviews extends React.Component<ISubmitSplitReviewsProps, ISubmitSplitReviewsState> {
  ...
  public async componentDidMount() {
    ...
    if (this.state.IsCreateMode) { }
      ...
      this.listSplitReviewsItemService = new ListItemService(this.props.AppContext, Config.ListNames.SplitReviews);
      this.hasEditItemPermission = await this.listSplitReviewsItemService.CheckCurrentUserCanEditItem(this.props.ItemID);
      const SplitReviewsDetails: PEPI_SplitReviews = await this.listSplitReviewsItemService.getItemUsingCAML(this.props.ItemID, [], undefined, Enums.ItemResultType.PEPI_SplitReviews);
      ...
      <div className={styles.SubmitSplitReviews}>
        <div className={styles.container}>
          ...
          <div className={styles.lblTopText}>
            ...
            <div className={styles.divCompetency}>
              <Label><b>INSTRUCTIONS:</b> Locate the ID number of the review you would like to split and enter it in the box labeled <b> Source Review ID</b>. Enter the<b> Hours to Review </b> and <b>Title of new Split Review </b> in the boxes below, then<b> click Create Split Review</b>.</Label>
            </div>
          ...
          <div className={styles.row}>
            <div> <hr></hr>
              ...
              <div className={styles.txtSourceReviewID}> <TextField onKeyPress={(e) => handleKeyPress(e)} resizable={false} multiline={false} value={this.state.SplitReviews.SourceReviewID} onChange={this.onChangeSourceReviewID} className={styles.Multilinetextarea}></TextField>   </div>

            </div><div>
              <div className={styles.lblHourstoReview}> <Label><b>Hours to Review </b></Label></div>
              <div className={styles.txtHourstoReview}><TextField onKeyPress={(e) => handleKeyPress(e)} resizable={false} multiline={false} value={this.state.SplitReviews.HourstoReview} onChange={this.onChangeHourstoReview} className={styles.Multilinetextarea}></TextField>   </div>
            </div>
          </div>
          ...
          <div className={styles.row}>
            ...
            <div className={styles.lblTitleOfSplitReview}>
              ...
              <Label className={styles.lblText}><b>(Example: Acme Software Implementation - Phase 1) </b><span style={{ color: '#ff0000' }}>*</span></Label>
            </div>
            <div className={styles.Newcol25left}>
              <TextField resizable={false} multiline={false} value={this.state.SplitReviews.Title} onChange={this.onChangeTitleofnewSplitReview} className={styles.Multilinetextarea}></TextField>
            </div>
          </div>
