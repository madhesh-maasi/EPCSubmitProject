...
export default class SubmitCombineReviews extends React.Component<ISubmitCombineReviewsProps, ISubmitCombineReviewsState> {
  ...
  public async componentDidMount() {
    ...
    if (this.state.IsCreateMode) { }
      ...
      this.ListItemService = new ListItemService(this.props.AppContext, Config.ListNames.CombineReviews);
      this.hasEditItemPermission = await this.ListItemService.CheckCurrentUserCanEditItem(this.props.ItemID);
      const CombineReviewsDetails: PEPI_CombineReviews = await this.ListItemService.getItemUsingCAML(this.props.ItemID, [], undefined, Enums.ItemResultType.PEPI_CombineReviews);
      this.setState({
        IsLoading: false,
  ...
  private async FillServiceLineOptions() {
    this.ListItemService = new ListItemService(this.props.AppContext, Config.ListNames.CombineReviews);
    let GetServiceLine = await this.ListItemService.getFieldChoices(Config.CombineReviewsListColumns.JobTitle);
          ...
          <div className={styles.container}>
            ...
            <div className={styles.row}>
              <div className={styles.lblTopText}>
                ...
                <div className={styles.divCompetency}>
                  <Label><b>INSTRUCTIONS:</b> Locate the ID numbers of the reviews you want to combine and enter them in the box labeled<b> Review IDs to combine</b>, separated by commas without spaces. Complete the <b>Title of Combined Review, Project Start & End Dates, Last Date Hours Billed</b>, and <b>Job Title</b>, then click <b>Create Combined Review</b>.</Label>
                </div>
                <hr className={styles.hr}></hr>
              </div>
              ...
              <div className={styles.row}>
                ...
                <div className={styles.txtReviewIDs}><TextField onKeyPress={(e) => handleKeyPress(e)}  resizable={false} multiline={false} value={this.state.CombineReviews.ReviewIDs} onChange={this.onChangeReviewIDs}  className={styles.Multilinetextarea}></TextField>   </div>
              </div>
              ...
              <div className={styles.row}>
                ...
                <div className={styles.txtTitle}><TextField resizable={false} multiline={false} value={this.state.CombineReviews.Title} onChange={this.onChangeTitleofCombinedReview} className={styles.Multilinetextarea}></TextField>   </div>
              </div>
              ...
              <div className={styles.row}>
                <div className={styles.lblTitle}>
                  ...
                  <Dropdown className={styles.dropServiceLine} options={this.ServiceLineOptions} selectedKey={this.state.CombineReviews.JobTitle} onChange={(e, selectedOption) => { this.onChangeJobTitle(selectedOption.text); }} />
                </div>
              </div>
