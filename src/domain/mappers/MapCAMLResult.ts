...
export default class MapCAMLResult extends ContextService {
    ...
    public static map(items: any, type: Enums.ItemResultType): any[] {
        ...
        items.forEach(item => {
            let result: any;
            switch (type) {
                case Enums.ItemResultType.PEPI_PEPIDetails: result = this.mapPEPIDetails(item);
                    break;
                case Enums.ItemResultType.PEPI_ItemID: result = this.mapPEPIItemID(item);
                    break;
                case Enums.ItemResultType.PEPI_QuestionText: result = this.mapPEPIQuestionText(item);
                    break;
                case Enums.ItemResultType.PEPI_CombineReviews: result = this.mapCombineReviews(item);
                    break;
                    case Enums.ItemResultType.PEPI_CombineAdmin: result = this.mapCombineAdmin(item);
                    break;
                case Enums.ItemResultType.PEPI_SplitReviews: result = this.mapSplitReviews(item);
                    break;
                    case Enums.ItemResultType.PEPI_SplitAdmin: result = this.mapSplitAdmin(item);
                    break;
                case Enums.ItemResultType.PEPI_SpecialReviews: result = this.mapSpecialReviews(item);
                    break;
            }
    ...
    private static mapPEPIDetails(item: any) {
        ...
        result.ID = item.ID;
        result.Reviewee = this.mapUser(item[Config.PEPIProjectsListColumns.RevieweeName]);
        result.LeadMD = this.mapUser(item[Config.PEPIProjectsListColumns.LeadMDName]);
        result.Reviewer = this.mapUser(item[Config.PEPIProjectsListColumns.ReviewerName]);
        result.Replaceme = this.mapUser(item[Config.PEPIProjectsListColumns.SubstituteUser]);
        result.ReplaceUsermailString = this.mapUser(item[Config.PEPIProjectsListColumns.SubstituteUser]).Email;
        ...
        result.OverallPerformance = item[Config.PEPIProjectsListColumns.OverallPerformance];


        result.RevertToReviewee = item[Config.PEPIProjectsListColumns.RevertToReviewee];
        result.RevertToReviewer = item[Config.PEPIProjectsListColumns.RevertToReviewer];
        result.JobTitle = item[Config.PEPIProjectsListColumns.JobTitle];
        // JobTitle
        //debugger;
        result.PerformanceDiscussion = item[Config.PEPIProjectsListColumns.PerformanceDiscussion] == "" ? new Date() : new Date(item[Config.PEPIProjectsListColumns.PerformanceDiscussion]);
    ...
    private static mapCombineReviews(item: any) {
        ...
        result.ProjectStartDate = item[Config.CombineReviewsListColumns.ProjectStartDate];
        result.ProjectEndDate = item[Config.CombineReviewsListColumns.ProjectEndDate];
    ...
    private static mapCombineAdmin(item: any) {
        ...
        result.ProjectStartDate = item[Config.CombineAdminListColumns.ProjectStartDate];
